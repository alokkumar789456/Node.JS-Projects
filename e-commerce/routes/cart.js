...
// Set storage engine for profile image uploads
...
// POST: Update Profile with Multer for image upload
router.post('/my/profile', upload.single('profileImage'), async (req, res) => {
    const token = req.cookies.token;
    const { name, email, phone, city, state, pincode, country, address } = req.body;
...
// Add to Cart
router.post('/add-to-cart', async (req, res) => {
    const { productId } = req.body;  // Expecting productId as a string between "1" and "20"
    const userId = req.user.id;

    try {
        const cartItem = new Cart({
            userId,
            productId  // No conversion needed, just store the string productId
        });
        await cartItem.save();
        res.send('Product added to cart');
    } catch (error) {
...
router.post('/remove-from-cart', async (req, res) => {
    ...
    try {
        await Cart.findOneAndDelete({
            userId,
            productId  // No conversion needed, productId remains a string
        });
        res.send('Product removed from cart');
    } catch (error) {
        console.error('Error removing product from cart:', error);
